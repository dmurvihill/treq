...
class _Response(proxyForInterface(IResponse)):
    ...
    def fail_for_status(self):
        """
        Fail with an error if the response has an error status.

        :rtype: A `Deferred` that fires with the response itself if the status
            is a known non-error code, or that calls its errback chain with a
            :class:`twisted.web.error.Error` otherwise.
        """
        if _is_error_code(self.code):
